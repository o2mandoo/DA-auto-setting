...
from experiments.db_fixtures.scripts.postgres_live_evidence import (
    collect_postgres_live_evidence,
    write_postgres_live_evidence,
)
...
def test_postgres_live_evidence_defaults_to_pending_without_fallback(tmp_path) -> None:
    output = tmp_path / "postgres_live_fixture_evidence.json"

    path = write_postgres_live_evidence(output, env={})
    ...
    assert payload["sql_plan"]["backend"] == "postgres"


def test_postgres_live_evidence_records_loaded_status_when_gate_passes() -> None:
    connection = _FakeConnection()

    evidence = collect_postgres_live_evidence(
        dsn="postgresql://localhost/semantic_fixture_postgres_scope_c",
