...
1. hosted CI pass log/URL still pending after workflow/interpreter/offline-env/suite/file-diagnostic and clean-evidence fixes;
...
1. **Hosted CI pass evidence pending** — local `make ci` passed, hosted runs exposed/fixed workflow interpreter/offline-env/suite/file-diagnostic and clean-evidence issues, and a passing hosted CI log/URL still must be attached.
...
Run the GitHub/hosted CI pipeline against the workflow/interpreter/offline-env/suite/file-diagnostic and clean-evidence fixes, attach the passing CI run URL/logs to the release packet, then create a signed/promoted release-candidate approval artifact. After that, regenerate the release packet and rerun this final evaluation.
